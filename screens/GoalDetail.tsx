...
import { Check, X, Calendar as CalendarIcon, ChevronLeft, ChevronRight, ArrowLeft, Trophy, Flame, Sparkles, Zap, List, MessageCircle, Send, Bot, Lock, Clock } from 'lucide-react';
...
export default function GoalDetail({ goal, onToggleTask, onBack, onUpdateTasks }: GoalDetailProps) {
  ...
  const maxDay = Math.max(0, ...taskDays);
  const endDate = new Date(startDate);
  // If maxDay is 0 (no numbered tasks), default to same day, else add days
  endDate.setDate(endDate.getDate() + (maxDay > 0 ? maxDay - 1 : 0));
  endDate.setHours(23, 59, 59, 999);

  const getDayTasks = (date: Date) => {
    if (!date) return { dayNum: 0, tasks: [] };
    ...
    const dayTasks = (goal.tasks || []).filter(t => {
       ...
       const prefix = `Day ${diffDays}`;
       return t.title.startsWith(`${prefix}:`) || t.title.startsWith(`${prefix} `);
    });
    
    return { dayNum: diffDays, tasks: dayTasks };
  };

  const getDayStatus = (date: Date) => {
    const { tasks, dayNum } = getDayTasks(date);
    
    // Check if in goal range (date >= startDate && date <= endDate)
    const inRange = date >= startDate && date <= endDate;
    
    if (!inRange) return 'out-of-range';
    if (dayNum < 1) return 'none'; 
  ...
  const handleSendMessage = async () => {
    ...
    const { dayNum, tasks } = getDayTasks(selectedDate);
    
    const response = await chatWithDayCoach(goal.title, goal.domain, dayNum, tasks, userMsg);
    ...
    if (response.suggested_tasks && response.suggested_tasks.length > 0 && onUpdateTasks) {
       onUpdateTasks(goal.id, dayNum, response.suggested_tasks);
      ...
      <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100">
        ...
        <div className="grid grid-cols-7 gap-y-2 gap-x-1">
          {days.map((date, i) => {
            ...
            const inRange = status !== 'out-of-range';
            
            let bgClass = 'bg-transparent text-slate-300';
            
            if (inRange) {
                if (status === 'complete') bgClass = 'bg-teal-500 text-white shadow-md shadow-teal-200';
                else if (status === 'partial') bgClass = 'bg-orange-100 text-orange-600 border border-orange-200';
                else if (status === 'missed') bgClass = 'bg-slate-50 text-slate-400 border border-slate-100';
                 ...
                 bgClass = 'opacity-30 text-slate-300'; // Out of goal duration
            }
            ...
            return (
              ...
              >
                {date.getDate()}
                {status !== 'none' && status !== 'out-of-range' && status !== 'missed' && status !== 'pending' && (
      ...
      <div className="min-h-full bg-slate-50 flex flex-col relative">
        <div className="bg-white sticky top-0 z-20 px-6 py-4 border-b border-slate-100 flex items-center gap-4 justify-between">
           <div className="flex items-center gap-4">
             ...
             <div>
               <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Day {dayNum}</div>
        ...
        {/* Coach Floating Button - High Z-Index and Positioned above Bottom Nav */}
        <button 
          onClick={() => setShowChat(true)}
          className="fixed bottom-24 right-6 bg-teal-600 text-white p-4 rounded-full shadow-xl shadow-teal-600/30 hover:scale-105 active:scale-95 transition-all z-40 flex items-center justify-center"
                   ...
                   <div className="flex items-center gap-3">
                     ...
                     <div>
                       ...
                       <p className="text-xs text-green-600 font-bold flex items-center gap-1">
                         <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span> Online • Day {dayNum} Context
                       </p>
                     </div>
                   </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50">
                   {chatMessages.length === 0 && (
                     <div className="text-center text-slate-400 my-8">
                       ...
                       <h4 className="font-bold text-slate-700 mb-2">How can I help with Day {dayNum}?</h4>
  ...
  return (
    <div className="min-h-full bg-slate-50 pb-20">
      ...
      <div className="p-6 space-y-6 animate-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Legend */}
        <div className="flex justify-center gap-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
